...
proto.getReward = function* (blockNumber) {
  try {
   const info = yield this.getLatestReward();
   return info
  } catch (err) {
    throw err;
  }
...
proto.getLatestReward = function* () {
  try {
    ...
    const data =  yield contract.call(opts)


    if (data.errorCode === 0) {
      const result = data.result
      if (result.error_code && result.error_code === 4) {
        return this._responseError(errors.QUERY_RESULT_NOT_EXIST, result.result);
      }

      if (result.error_code && result.error_code > 0 && result.error_code !== 4) {
        return this._responseError(errors.FAIL);
      }

      const nodeList = result['query_rets']
      const info = JSON.parse(nodeList[0]['result']['value'])

      const validatorsInfo = info.rewards.validators
      const kolsInfo = info.rewards.kols
      const validatorList = []
      const kolsList = []

      Object.keys(validatorsInfo).forEach(address => {
        const obj = {}
        obj.address = address
        obj.reward = validatorsInfo[address]
        validatorList.push(obj)
      })

      Object.keys(kolsInfo).forEach(address => {
        const obj = {}
        obj.address = address
        obj.reward = kolsInfo[address]
        kolsList.push(obj)
      })
